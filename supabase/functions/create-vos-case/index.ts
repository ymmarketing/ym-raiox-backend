...
function cors(origin: string | null) {
  const allow = origin && ALLOWED_ORIGINS.has(origin) ? origin : ORIGIN_E4;
  return {
...
Deno.serve(async (req: Request) => {
  ...
  const { data: access, error: accessError } = await supabase.from("vos_internal_access").select("role,active").eq("email", claims.email).maybeSingle();
  ...
  if (!access || access.active !== true || !["ADMIN", "APLICADOR"].includes(access.role)) {
    await supabase.from("vos_access_audit").insert({ email: claims.email, role: access?.role || null, event: "CREATE_CASE_DENIED", metadata: { intake_id: intakeId, sub: claims.sub } });
    return reply(403, { ok: false, error: "forbidden" }, origin);
  }

  const { data, error } = await supabase.rpc("vos_create_case_from_intake", { p_intake_id: intakeId, p_created_by: claims.email });
  if (error) {
  ...
  await supabase.from("vos_access_audit").insert({ email: claims.email, role: access.role, event: "CREATE_CASE_OK", metadata: { intake_id: intakeId, case_id: data } });
