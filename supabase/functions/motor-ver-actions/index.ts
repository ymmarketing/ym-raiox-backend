...
Deno.serve(async(req:Request)=>{
  ...
  try{
    if(action==="UPDATE_DESTINATION"){
      const patch={destination_short_term:text(body.destination_short_term,2500)||null,destination_success_signal:text(body.destination_success_signal,2500)||null,notes:text(body.notes,4000)||null,updated_by:c.email};
      ...
      const patch:any={observation:text(body.observation,5000)||null,evidence_summary:text(body.evidence_summary,5000)||null,classification:body.classification||null,confidence:body.confidence||null,remaining_validation:text(body.remaining_validation,3000)||null,human_status:humanStatus,validated_by:humanStatus==="VALIDADO"?c.email:null,validated_at:humanStatus==="VALIDADO"?new Date().toISOString():null};
      ...
      if(humanStatus==="VALIDADO")await sb.from("vos_validations").insert({case_id:caseId,target_type:"P8",target_id:q.data.id,decision:"VALIDAR",rationale:text(body.rationale,2500)||"Validação humana registrada no Motor VOS.",validated_by:c.email});
      ...
      const q=await sb.from("vos_evidence").insert({case_id:caseId,evidence_type:body.evidence_type,title,content:text(body.content,8000)||null,source_ref:sourceRef,source_url:text(body.source_url,2000)||null,observed_at:body.observed_at||null,p8_code:body.p8_code||null,reliability:body.reliability||null,created_by:c.email}).select().single();if(q.error)throw q.error;result=q.data;
      ...
      const q=await sb.from("vos_ver_entries").update({human_status:status,validated_by:c.email,validated_at:new Date().toISOString()}).eq("id",body.entry_id).eq("case_id",caseId).select().single();if(q.error)throw q.error;result=q.data;
      await sb.from("vos_validations").insert({case_id:caseId,target_type:"VER_ENTRY",target_id:body.entry_id,decision,rationale:text(body.rationale,2500)||"Decisão humana registrada no Motor VOS.",validated_by:c.email});
      ...
      const q=await sb.from("vos_hypotheses").insert({case_id:caseId,statement,p8_code:body.p8_code||null,origin:"HUMAN",status:"SUGERIDA",confidence:body.confidence||null,created_by:c.email}).select().single();if(q.error)throw q.error;result=q.data;
      ...
      const q=await sb.from("vos_hypothesis_tests").insert({hypothesis_id:body.hypothesis_id,test_description:desc,method:text(body.method,3000)||null,expected_evidence:text(body.expected_evidence,3000)||null,result_summary:text(body.result_summary,5000)||null,result_classification:body.result_classification||null,tested_by:body.result_classification?c.email:null,tested_at:body.result_classification?new Date().toISOString():null}).select().single();if(q.error)throw q.error;result=q.data;
      ...
      const patch:any={status:body.status,validated_by:c.email,validated_at:new Date().toISOString()};if(body.confidence){if(!CONFIDENCE.has(body.confidence))return reply(400,{ok:false,error:"invalid_confidence"},origin);patch.confidence=body.confidence;}
      ...
      await sb.from("vos_validations").insert({case_id:caseId,target_type:"HYPOTHESIS",target_id:body.hypothesis_id,decision,rationale:text(body.rationale,3000)||"Decisão humana registrada após teste de hipótese.",validated_by:c.email});
      ...
      const q=await sb.from("vos_conclusions").insert({case_id:caseId,conclusion_type:conclusionType,statement,confidence:body.confidence,uncertainty:text(body.uncertainty,3500)||null,impact_on_destination:text(body.impact_on_destination,3500)||null,human_validated_by:c.email,human_validated_at:new Date().toISOString()}).select().single();if(q.error)throw q.error;result=q.data;
      await sb.from("vos_validations").insert({case_id:caseId,target_type:"CONCLUSION",target_id:q.data.id,decision:"VALIDAR",rationale:text(body.rationale,3000)||"Conclusão registrada e validada explicitamente por aplicadora humana.",validated_by:c.email});
      ...
      const q=await sb.from("vos_gates").update({status:gateStatus,justification,remaining_conditions:text(body.remaining_conditions,4000)||null,validated_by:c.email,validated_at:new Date().toISOString()}).eq("case_id",caseId).eq("gate_code","VER_GATE").select().single();if(q.error)throw q.error;result=q.data;
      await sb.from("vos_validations").insert({case_id:caseId,target_type:"GATE",target_id:q.data.id,decision:gateStatus==="APROVADO"?"VALIDAR":"REJEITAR",rationale:justification,validated_by:c.email});
      await sb.from("vos_cases").update({status:gateStatus==="APROVADO"?"VER_VALIDADO":"VER_AGUARDANDO_VALIDACAO",updated_by:c.email}).eq("id",caseId);
  ...
  await sb.from("vos_access_audit").insert({email:c.email,role:access.role,event:"VER_ACTION",metadata:{case_id:caseId,action}});
