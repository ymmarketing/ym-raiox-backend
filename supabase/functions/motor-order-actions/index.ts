...
Deno.serve(async(req:Request)=>{
  ...
  try{
    if(action==="CREATE_CANDIDATE"){
      ...
      const {data,error}=await sb.from("vos_order_candidates").insert({
        ...
        not_now:body.not_now===true,human_status:"PENDENTE",sequence_order:null,created_by:c.email
      }).select().single();
      ...
      const {data,error}=await sb.from("vos_order_candidates").update({human_status:"VALIDADO",sequence_order:seq,validated_by:c.email,validated_at:new Date().toISOString()}).eq("id",body.candidate_id).eq("case_id",caseId).select().single();
      if(error)throw error;
      await sb.from("vos_validations").insert({case_id:caseId,target_type:"ORDER_CANDIDATE",target_id:body.candidate_id,decision:"VALIDAR",rationale,validated_by:c.email});
      result=data;
    } else if(action==="REJECT_CANDIDATE"){
      if(!uuid(body.candidate_id))return reply(400,{ok:false,error:"invalid_candidate_id"},origin);
      const rationale=text(body.validation_rationale,4000);if(!rationale)return reply(400,{ok:false,error:"validation_rationale_required"},origin);
      const {data,error}=await sb.from("vos_order_candidates").update({human_status:"REJEITADO",sequence_order:null,validated_by:c.email,validated_at:new Date().toISOString()}).eq("id",body.candidate_id).eq("case_id",caseId).select().single();
      if(error)throw error;
      await sb.from("vos_validations").insert({case_id:caseId,target_type:"ORDER_CANDIDATE",target_id:body.candidate_id,decision:"REJEITAR",rationale,validated_by:c.email});
      result=data;
  ...
  await sb.from("vos_access_audit").insert({email:c.email,role:access.role,event:"ORDER_ACTION",metadata:{case_id:caseId,action,target_id:result?.id||null}});
  if(action==="VALIDATE_CANDIDATE")await sb.from("vos_cases").update({status:"ORDENAR_PREPARADO",updated_by:c.email}).eq("id",caseId).eq("status","VER_VALIDADO");
