...
function numOrNull(v:unknown){if(v===null||v===undefined||v==='')return null;const n=Number(v);return Number.isFinite(n)?n:null;}
...
Deno.serve(async(req:Request)=>{
  ...
  if(req.method==='GET'){
    const [{data:opps,error:oe},{data:activities,error:acte},{data:history,error:he},{data:intakes,error:ie},{data:cases,error:ce}]=await Promise.all([
      sb.from('crm_opportunities').select('id,current_stage,stage_entered_at,next_action,next_action_due_at,next_action_updated_at,recommended_route,route_rationale,route_validated_by,route_validated_at,proposal_value,won_at,lost_at,loss_reason,source_intake_id,source_case_id,owner_email,notes,updated_at,created_at,initial_reading_status,initial_reading_url,initial_reading_date,contact_status,contact_date,contact_result,import_rank,trigger_moment,contact:crm_contacts(id,client_ref,external_key,name,business_name,email,phone,source,owner_email,notes,active,city_state,segment,decision_maker,website_url,linkedin_url,instagram_url,lead_class,lead_score,lead_confidence,foundation_year,recommended_channel,approach_angle,public_signal,opportunity_to_validate,offer_summary,research_source,duplicate_audit,source_dataset,source_row_ref)').order('import_rank',{ascending:true,nullsFirst:false}).order('updated_at',{ascending:false}).limit(300),
      sb.from('crm_activities').select('id,opportunity_id,activity_type,content,due_at,completed_at,created_by,created_at').order('created_at',{ascending:false}).limit(2000),
      sb.from('crm_stage_history').select('id,opportunity_id,from_stage,to_stage,reason,changed_by,changed_at').order('changed_at',{ascending:false}).limit(2000),
      sb.from('raiox_intakes').select('id,client_ref,score_overall,score_status,created_at').order('created_at',{ascending:false}).limit(300),
      sb.from('vos_cases').select('id,source_intake_id,client_ref,business_name,status,created_at').order('created_at',{ascending:false}).limit(300)
    ]);
  ...
  try{
    if(action==='CREATE_LEAD'){
      ...
      if(!uuid(body.opportunity_id))return reply(400,{ok:false,error:'invalid_opportunity_id'},origin);const status=text(body.contact_status,160);if(status&&!CONTACT_STATUSES.includes(status))return reply(400,{ok:false,error:'invalid_contact_status'},origin);const patch={contact_status:status||null,contact_date:nullableDate(body.contact_date),contact_result:text(body.contact_result,5000)||null,updated_by:c.email,updated_at:new Date().toISOString()};const {data,error}=await sb.from('crm_opportunities').update(patch).eq('id',body.opportunity_id).select().single();if(error)throw error;result=data;
      ...
      const leadScore=numOrNull(body.lead_score);if(leadScore!==null&&(leadScore<0||leadScore>100))return reply(400,{ok:false,error:'lead_score_out_of_range'},origin);
      const contactPatch:any={name:text(body.name,500)||null,business_name:text(body.business_name,500)||null,email:text(body.email,500)||null,phone:text(body.phone,160)||null,source:text(body.source,500)||null,owner_email:text(body.contact_owner_email,500)||null,notes:text(body.contact_notes,10000)||null,active:body.active!==false,city_state:text(body.city_state,300)||null,segment:text(body.segment,800)||null,decision_maker:text(body.decision_maker,500)||null,website_url:text(body.website_url,2000)||null,linkedin_url:text(body.linkedin_url,2000)||null,instagram_url:text(body.instagram_url,2000)||null,lead_class:text(body.lead_class,200)||null,lead_score:leadScore,lead_confidence:text(body.lead_confidence,200)||null,foundation_year:text(body.foundation_year,100)||null,recommended_channel:text(body.recommended_channel,200)||null,approach_angle:text(body.approach_angle,3000)||null,public_signal:text(body.public_signal,5000)||null,opportunity_to_validate:text(body.opportunity_to_validate,5000)||null,offer_summary:text(body.offer_summary,5000)||null,research_source:text(body.research_source,3000)||null,duplicate_audit:text(body.duplicate_audit,3000)||null,source_dataset:text(body.source_dataset,500)||null,source_row_ref:text(body.source_row_ref,500)||null,updated_at:new Date().toISOString()};
      const {data:contact,error:contactErr}=await sb.from('crm_contacts').update(contactPatch).eq('id',opp.contact_id).select().single();if(contactErr)throw contactErr;
      const oppPatch:any={notes:text(body.opportunity_notes,10000)||null,proposal_value:numOrNull(body.proposal_value),trigger_moment:text(body.trigger_moment,3000)||null,owner_email:text(body.opportunity_owner_email,500)||null,loss_reason:text(body.loss_reason,3000)||null,updated_by:c.email,updated_at:new Date().toISOString()};
      const {data:oppUpdated,error:oppUpdateErr}=await sb.from('crm_opportunities').update(oppPatch).eq('id',body.opportunity_id).select().single();if(oppUpdateErr)throw oppUpdateErr;result={contact,opportunity:oppUpdated};
      ...
      const {data:opp,error:opErr}=await sb.from('crm_opportunities').select('id,contact_id,current_stage,source_intake_id,source_case_id,contact:crm_contacts(name,business_name)').eq('id',body.opportunity_id).maybeSingle();if(opErr)throw opErr;if(!opp)return reply(404,{ok:false,error:'opportunity_not_found'},origin);
      const label=text((opp as any).contact?.business_name|| (opp as any).contact?.name||'',500);const confirm=text(body.confirm_label,500);if(!confirm||confirm.toLowerCase()!==label.toLowerCase())return reply(400,{ok:false,error:'delete_confirmation_mismatch',detail:'Digite exatamente o nome do negócio/contato para confirmar.'},origin);
      const deletedMeta={opportunity_id:opp.id,contact_id:opp.contact_id,label,current_stage:opp.current_stage,source_intake_id:opp.source_intake_id,source_case_id:opp.source_case_id};
      const {error:delErr}=await sb.from('crm_opportunities').delete().eq('id',body.opportunity_id);if(delErr)throw delErr;
      const {count,error:countErr}=await sb.from('crm_opportunities').select('id',{count:'exact',head:true}).eq('contact_id',opp.contact_id);if(countErr)throw countErr;
      let contact_deleted=false;if((count||0)===0){const {error:cdErr}=await sb.from('crm_contacts').delete().eq('id',opp.contact_id);if(cdErr)throw cdErr;contact_deleted=true;}
      await sb.from('vos_access_audit').insert({email:c.email,role:access.role,event:'CRM_DELETE',metadata:{...deletedMeta,contact_deleted}});result={...deletedMeta,contact_deleted};
