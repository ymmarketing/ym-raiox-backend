...
export function analyzeMetricPeriod(value){
  ...
  if(!startRaw || !endRaw || Number.isNaN(start.getTime()) || Number.isNaN(end.getTime()) || end<start){
    return {start:null,end:null,days:null,months_30d:null};
  }
  const days=Math.floor((end-start)/86400000)+1;
  return {start:startRaw,end:endRaw,days,months_30d:Number((days/30).toFixed(2))};
}

export function assessDecisionSufficiency(raw={}){
  const period=analyzeMetricPeriod(raw.metric_period);
  ...
  const conversionSample=leads===null ? 'unknown' : leads<10 ? 'very_low' : leads<30 ? 'limited' : 'usable';
  const acquisitionSignal=leadsPer30===null
    ? 'unknown'
    : period.days>=30 && leadsPer30<1
      ? 'critical_scarcity'
      : leads<10
        ? 'low_volume'
        : 'observable';
  const overall=conversionSample==='usable' ? 'usable' : conversionSample==='limited' ? 'limited' : 'insufficient';
  return {
    overall,
    overall_label:overall==='usable'?'Suficiente para leitura inicial':overall==='limited'?'Limitada — exige cautela':'Insuficiente para concluir conversão',
    period,
    sample:{leads,customers,conversion_sample:conversionSample},
    acquisition:{leads_per_30d:leadsPer30,signal:acquisitionSignal},
    rules:[
      'Preenchimento completo não significa amostra suficiente.',
      'Volume zero ou muito baixo é evidência sobre aquisição quando o período foi informado.',
      'A causa da escassez deve ser investigada; não pode ser presumida.'
    ],
  };
}
...
export function calculateBusinessKpis(raw = {}) {
  ...
  return {
    ...
    opportunities,
    proposals,
    metric_period_days:period.days,
    leads_per_30d:leadsPer30,
  };
}
...
export function assessDataQuality(input = {}) {
  ...
  const decisionSufficiency=assessDecisionSufficiency(input);
  ...
  const canProjectFinancial=grade==='A' &&
    ...
    decisionSufficiency.overall!=='insufficient';
  ...
  return {
    ...
    can_build_targets:grade!=='C' && decisionSufficiency.overall!=='insufficient',
    can_project_financial:canProjectFinancial,
    target_mode:grade==='C' || decisionSufficiency.overall==='insufficient' ? 'bloqueado' : grade==='A' ? 'faixa_completa' : 'direcional_com_limitacoes',
    rule: grade==='C' || decisionSufficiency.overall==='insufficient'
