...
function headers() {
  return {
    ...
    'User-Agent': 'YM-RaioX-Estrategico/2.2',
 ...
 *
 * Após o pagamento, o cliente volta DIRETO para o app oficial V2.2.
 * O próprio app revalida a sessão no backend antes de mostrar o questionário,
 * evitando corrida entre o redirect do Asaas e o webhook de confirmação.
 */
export async function criarCobranca({ customerId, valor, descricao, ref, vencimentoDias = 3 }) {
  ...
  if (siteUrl) {
    corpo.callback = {
      successUrl: `${siteUrl}/raio-x-app.html?ref=${encodeURIComponent(ref)}`,
      autoRedirect: true,
    };
  }
