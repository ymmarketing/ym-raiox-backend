...
function configuracao() {
  return {
    ...
    ambiente: String(process.env.ASAAS_ENV || 'production').trim().toLowerCase(),
    permitirProducao: String(process.env.CODIGO_EXECUCAO_MESTRE_ALLOW_PRODUCTION || '').trim().toLowerCase() === 'true',
  };
}

export function statusMestreExecucao(agora = new Date()) {
  const { hash, expiraEm, ambiente, permitirProducao } = configuracao();
  const expiraMs = Date.parse(expiraEm);
  const hashValido = /^[a-f0-9]{64}$/.test(hash);
  const dataValida = Number.isFinite(expiraMs);
  const ambientePermitido = ambiente !== 'production' || permitirProducao;
  return {
    ativo: ambientePermitido && hashValido && dataValida && agora.getTime() < expiraMs,
    expiraEm: dataValida ? new Date(expiraMs).toISOString() : null,
    permitidoEmProducao: permitirProducao,
  };
}
