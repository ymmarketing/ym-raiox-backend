...
export async function generateVosIntelligenceReport(intake,{fetch_impl=globalThis.fetch,api_key=process.env.OPENAI_API_KEY||''}={}){
  ...
  const priorities=orderPriorities(diagnostic.priorities||[]).slice(0,3);
  ...
  return {
    report:{
      ...
      playbook,
      content_week,
      source_analysis:{links:linkAudit.map(({content,...link})=>link),images:(intake.images||[]).map(image=>({id:image.id,name:image.name,status:'analisado_na_execucao'}))},
