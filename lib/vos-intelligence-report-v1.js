    ...
    report:{
      ...
      source_analysis:{
        ...
        images:(intake.images||[]).map(image=>{
          ...
          return {id:image.id,name:image.name,status:observation?.access_status||'parcial',evidence_basis:observation?.evidence_basis||'visual_analysis'};
        }),
        coverage:diagnostic.source_coverage||[],
        observations:diagnostic.source_observations||[],
        audit:{provider:analysis.audit.provider,model:analysis.audit.model,web_search_calls:analysis.audit.web_search_calls,links_received:linkAudit.length,images_received:(intake.images||[]).length},
      },
