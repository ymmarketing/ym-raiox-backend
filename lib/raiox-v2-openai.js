/**
 * RX_REPORT_2.1 — análise multimodal do Raio-X Estratégico YM.
...
export const REPORT_VERSION_V2 = 'RX_REPORT_2.1';
...
2. Painel de Score Estratégico: nota geral + 6 frentes, de 0 a 10.
3. Conexões: o que nos levou à leitura.
4. Jornada: onde flui, onde há atenção, onde falta evidência.
5. 3–5 achados: aprofundamento sem repetição.
6. Leitura por frentes: síntese curta.
7. Prioridades: no máximo 3.
8. Direção 0–30 / 31–60 / 61–90 dias: orientação, não plano fechado.
9. O que ainda não pode ser afirmado.
10. Fechamento em uma frase.

SCORES — REGRA OBRIGATÓRIA
Cada uma das 6 frentes deve receber uma nota de 0.0 a 10.0, com uma casa decimal.
O score mede MATURIDADE ESTRUTURAL OBSERVADA no momento do diagnóstico — não qualidade da empresa, potencial futuro, faturamento ou valor profissional.
As 6 frentes são:
1. Posicionamento & Oferta
2. Presença & Descoberta
3. Conteúdo & Autoridade
4. Jornada & Conversão
5. Relacionamento & Comercial
6. Operação & Sustentação

Régua de referência:
- 0.0–2.9 = Inicial: estrutura ausente, muito fragmentada ou dependente de improviso.
- 3.0–4.9 = Em construção: existem ativos isolados, mas faltam conexões importantes.
- 5.0–6.9 = Funcional: base real funcionando, porém com lacunas relevantes de consistência, conexão ou mensuração.
- 7.0–8.4 = Estruturado: frente bem construída e conectada, com oportunidades específicas de evolução.
- 8.5–10.0 = Consolidado: estrutura forte, coerente, recorrente e sustentada por evidências.

Regras dos scores:
- Não dê nota baixa apenas porque um link, print ou métrica não foi enviado.
- Quando faltar evidência, use os fatos do questionário e reduza a CONFIANÇA DO SCORE, não invente certeza.
- Score alto exige evidência positiva; ausência de problema não é suficiente.
- Score baixo exige sinais concretos de fragilidade, ausência estrutural ou dependência excessiva de processo manual.
- A justificativa do score deve caber em uma frase curta e específica.
- Os scores precisam ser coerentes com os achados, prioridades e leitura por frentes.
- Não use benchmarks externos para calcular a nota.
...
const REPORT_SCHEMA = {
  ...
  properties: {
    report_version: { type:'string', enum:['RX_REPORT_2.1'] },
    summary: {
    ...
    fronts:{
      type:'array',minItems:6,maxItems:6,
      items:{type:'object',additionalProperties:false,required:['front','score','score_rationale','score_confidence','asset','signal','reading','sources'],
        properties:{
          ...
          score_confidence:{type:'string',enum:['forte','consistente','a_validar']},
...
function enrichScores(report){
  const fronts=Array.isArray(report?.fronts)?report.fronts:[];
  for(const x of fronts){
    const n=Number(x?.score);
    x.score=Number.isFinite(n)?Number(Math.max(0,Math.min(10,n)).toFixed(1)):0;
  }
  const vals=fronts.map(x=>Number(x.score)).filter(Number.isFinite);
  report.score_overall=vals.length?Number((vals.reduce((a,b)=>a+b,0)/vals.length).toFixed(1)):0;
  return report;
}
...
${JSON.stringify(payload)}`
  ...
  const body={
    ...
    store:false,
    text:{format:{type:'json_schema',name:'ym_raiox_report_v21',strict:true,schema:REPORT_SCHEMA}}
  };
