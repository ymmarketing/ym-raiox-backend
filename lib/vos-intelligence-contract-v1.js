...
export function buildDiagnosticEnvelope({questionnaire_version,business_name,company={},answers={},complements={},metrics={},external_sources=[]}={}){
  ...
  const answerEvidence=buildEvidenceIndex({questionnaire_version,answers,complements});
  const acquisitionMetrics=new Set(['metric_period','visits','leads','acquisition_spend','customers','capacity_monthly']);
  const metricEvidence=Object.entries(raw_metrics).filter(([,value])=>present(value)).map(([key,value])=>({
    source_id:key==='metric_period'?'METRIC_PERIOD':`METRIC_${key.toUpperCase()}`,
    text:`${key}: ${clean(value,500)}`,
    role:'declared_metric',
    pillars:[acquisitionMetrics.has(key)?PILLARS.AQUISICAO:PILLARS.OPERACAO],
    ps:['Performance'],
    evidence_type:'declared_metric',
  }));
  const evidence=[...answerEvidence,...metricEvidence];
  ...
  return {
    ...
    external_sources:(external_sources||[]).map(item=>({
      id:clean(item?.id,40),type:clean(item?.type,40),status:clean(item?.status,30),
      url:clean(item?.url,1500),context:clean(item?.context,1200),content:clean(item?.content,18000),
    })).filter(item=>item.id),
